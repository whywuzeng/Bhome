...
public abstract class ChatFragment<InitModel,InitMaker>
     ...
     * 判断是否铺满一屏幕
     */
    private boolean isNoFullScreen=false;
    ...
    protected void fixStackFromEnd(final LinearLayoutManager linearLayoutManager){

        mRecyclerView.addOnChildAttachStateChangeListener(new RecyclerView.OnChildAttachStateChangeListener() {
            @Override
            public void onChildViewAttachedToWindow(final View view) {
                Log.e(TAG, "onChildViewAttachedToWindow: 超过屏幕linearLayoutManager.getPosition(view)"+linearLayoutManager.getPosition(view) );
                /**
                 * 只显示 屏幕内的
                 */
                if (linearLayoutManager.getPosition(view)==0)
                {
                    view.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                        ...
                        public void onGlobalLayout() {
                            int top = view.getTop();
                             if (top>Math.min(view.getHeight(),Ui.dipToPx(getResources(),48f)))
                             {
    ...
    protected void initRecycleView(){
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                /**
                 * 设置为  弹出软键盘会 重置setStackEnd false
                 */
