...
public class CircleFlowIndicator extends View implements FlowIndicator,
	...
	protected void onDraw(Canvas canvas) {
		...
		// this is the amount the first circle should be offset to make
		// the
		...
		float cx = 0;
		if (flowWidth != 0) {
			// Draw the filled circle according to the current
			// scroll
			cx = (currentScroll * circleSeparation) / flowWidth;
		}
		// The flow width has been upadated yet. Draw the default
		// position
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.taptwo.android.widget.ViewFlow.ViewSwitchListener#onSwitched(
	...
	private int measureWidth(int measureSpec) {
		...
		// Calculate the width according the views count
			...
			// Respect AT_MOST value if that was what is called for
			// by
			// measureSpec
	...
	private void resetTimer() {
		// Only set the timer if we have a timeout of at least 1
		// millisecond
		if (fadeOutTime > 0) {
		...
		protected Void doInBackground(Void... arg0) {
			while (_run) {
				try {
					...
					// Check if we've reached the fade out
					// time
