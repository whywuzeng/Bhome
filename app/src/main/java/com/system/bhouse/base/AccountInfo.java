...
//    }
    ...
    public static void saveFriends(Context ctx, ArrayList<UserObject> data, UsersListFragment.Friend type) {
        String name = friendTypeToFileName(type);
        new DataCache<UserObject>().save(ctx, data, name);
    }

    public static ArrayList<UserObject> loadFriends(Context ctx, UsersListFragment.Friend type) {
        String fileName = friendTypeToFileName(type);
        return new DataCache<UserObject>().load(ctx, fileName);
    }

    private static String friendTypeToFileName(UsersListFragment.Friend type) {
        if (type == UsersListFragment.Friend.Follow) {
