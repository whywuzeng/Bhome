...
public class MyJumpingBeans  {
    ...
    private final MyJumpingBeansSpan[] jumpingBeans;
    private final WeakReference<TextView> textView;

    private MyJumpingBeans(@NonNull MyJumpingBeansSpan[] beans, @NonNull TextView textView) {
    ...
    public void stopJumping() {
        for (MyJumpingBeansSpan bean : jumpingBeans) {
            if (bean != null) {
                bean.teardown();
            }
        }
    ...
    private static CharSequence removeJumpingBeansSpansFrom(Spanned text) {
        ...
        for (Object span : spans) {
            if (!(span instanceof MyJumpingBeansSpan)) {
    ...
    public static class Builder {
        ...
        public MyJumpingBeans build() {
            ...
            return new MyJumpingBeans(spans, textView);
        }

        private MyJumpingBeansSpan[] getJumpingBeansSpans(SpannableStringBuilder sbb) {
            MyJumpingBeansSpan[] spans;
            if (waveCharDelay == -1) {
                waveCharDelay = loopDuration / (3 * (endPos - startPos));
            }

            spans = new MyJumpingBeansSpan[endPos - startPos];
            for (int pos = startPos; pos < endPos; pos++) {
                MyJumpingBeansSpan jumpingBean =
                        new MyJumpingBeansSpan(textView, loopDuration, pos - startPos, waveCharDelay, animRange);
        ...
        private MyJumpingBeansSpan[] buildSingleSpan(SpannableStringBuilder sbb) {
            MyJumpingBeansSpan[] spans = new MyJumpingBeansSpan[]{new MyJumpingBeansSpan(textView, loopDuration, 0, 0, animRange)};
