package com.system.bhouse.bhouse.CommonTask.TechnologyExecution.ComponentTakeOff;
...
import com.system.bhouse.bhouse.CommonTask.TechnologyExecution.ComponentTakeOff.Bean.ComponentTakeOffBean;
import com.system.bhouse.bhouse.CommonTask.TechnologyExecution.ComponentTakeOff.Bean.ComponentTakeOffBeanSection;
...
public class ComponentTakeOffContentMessageActivity extends BaseContentMessageActivity implements  GroupItem.onChildItemClickListener, onMutiDataSetListener ,BaseQuickAdapter.OnItemChildClickListener,BaseContentMessageActivity.SetOnAddItemClickListener {

    public static final String TAG = "comtaskcontentmessageactivity";
    private static final String module_name = "构件脱模";
    ...
    private ArrayList<ComponentTakeOffBean> comTaskBeans = new ArrayList<>();
    private ArrayList<ComponentTakeOffBeanSection> assignmentBeanSectionArrayList=new ArrayList<>() ;
    ...
    private ComponentTakeOffSectionAdapter componentTakeOffSectionAdapter;
    ...
    public void initComTaskActivity() {
        ...
        String[] stringArray = getResources().getStringArray(R.array.component_takeoff_itemsection_order);

        componentTakeOffSectionAdapter = new ComponentTakeOffSectionAdapter(R.layout.activity_comtask_content_layout_item, R.layout.layout_home_recommend_empty_noheight, R.layout.comtask_content_item_footer,assignmentBeanSectionArrayList,stringArray,mStatus);

        listView.setNestedScrollingEnabled(false);

        listView.setAdapter(componentTakeOffSectionAdapter);
        listView.addItemDecoration(new TimeLineItemTopBottomDecoration());
        componentTakeOffSectionAdapter.setOnItemChildClickListener(this);
...
//            this.comTaskBeans = savedInstanceState.getParcelableArrayList(STATE_COMTASK);
    ...
    private void initStatusData(){
        ...
        if (!TextUtils.isEmpty(receiptHnumber)) {
            if (getComtaskSize()) {
                for (ComponentTakeOffBean bean : comTaskBeans) {
    ...
    private void TopListViewInit() {
        ...
        ArrayList<ComponentTakeOffContentMessageActivity.KeyType> keyTypes = new ArrayList<>();
        ComponentTakeOffContentMessageActivity.KeyType keyType = null;
        for (int i = 0; i < Const.LETTERS.length; i++) {
            keyType = new ComponentTakeOffContentMessageActivity.KeyType();
    ...
    private void ifStateForOrderId() {
        ...
        }else if (comTaskBeans.get(0).getStatus().trim().equals(Const.CHECK_STATUS))
        {
                ...
                checkStatusBean.setVisDeleteBtn(true);
     ...
     * @return
     */
    protected ArrayList<SortChildItem.ViewModel> makeChlidData(ComponentTakeOffContentMessageActivity.KeyType data, StatusBean mStatus) {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (viewEditText.getText() != null) {
                    ...
                    if (TextUtils.isEmpty(text)) {
                        T.showfunShort(ComponentTakeOffContentMessageActivity.this, "备注不能为空");
                    }
                    else {
            ...
            if (extraPosition==-1) {
                ApiWebService.Get_Production_order_Mould_bypoid_Json(this, new ApiWebService.SuccessCall() {
                    ...
                    public void SuccessBack(String result) {
                        ...
                        ArrayList<ComponentTakeOffBean> loadingcarbean = App.getAppGson().fromJson(result, new TypeToken<List<ComponentTakeOffBean>>() {
                        }.getType());
...
//                            T.showShort(ComponentTakeOffContentMessageActivity.this,getResources().getString(R.string.Qrcode_result));
                        ...
                        //清空 二维码为空的
                        for (ComponentTakeOffBean receBean : comTaskBeans) {
                        ...
                        ArrayList<ComponentTakeOffBean> clone =(ArrayList<ComponentTakeOffBean>)comTaskBeans.clone();
                        comTaskBeans.clear();
                        comTaskBeans.addAll(removeDupliById(clone));
                        ClearAssignMentSectionArrayList();
                        componentTakeOffSectionAdapter.notifyDataSetChanged();
                    }
...
//
//            ClearAssignMentSectionArrayList();
//            componentTakeOffSectionAdapter.notifyItemChanged(extraPosition);
        }
    }

    public static ArrayList<ComponentTakeOffBean> removeDupliById(List<ComponentTakeOffBean> persons) {
        Set<ComponentTakeOffBean> personSet = new TreeSet<>((o1, o2) -> {
    ...
    private void getStaffData(int position) {
        ApiWebService.Get_Production_order_Per_Member_Json(this, new ApiWebService.SuccessCall() {
            ...
            public void SuccessBack(String result) {
                ...
                CommonPickerActivity_.intent(ComponentTakeOffContentMessageActivity.this).title("职员信息").bProBOMs(bProBOMs).Position(position).LayoutID(R.layout.commonpicker_item).startForResult(RESULT_SORTITEM_SELECTPROJECT);
...
//        }, orderId, componentQr, comTaskBeans.get(0).ID);
    ...
    private void testData() {
        comTaskBeans.clear();
        ApiWebService.Get_Production_order_yhy_Out_MouldView_Json_poidprid_QR_Code(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                ArrayList<ComponentTakeOffBean> tomTaskBeans = App.getAppGson().fromJson(result, new TypeToken<List<ComponentTakeOffBean>>() {}.getType());
                //为空就创建一个新的空对象
                if (tomTaskBeans.isEmpty()) {
                    //为头recycleView 设置空数据
                    comTaskBeans.add(new ComponentTakeOffBean());
                }
                else {
            ...
            public void ErrorBack(String error) {
                ...
                //设置Empty  ErrorView
                componentTakeOffSectionAdapter.setEmptyView(errorView);
            }
        },orderId,componentQr);
    }
...
//        treeRecyclerAdapter.getItemManager().addItems(datas);
    ...
    private void ClearAssignMentSectionArrayList() {
        ...
        for (ComponentTakeOffBean bean : comTaskBeans) {
            assignmentBeanSectionArrayList.add(new ComponentTakeOffBeanSection(bean));
        }
        assignmentBeanSectionArrayList.add(new ComponentTakeOffBeanSection(false, "", false, true, ""));
    }
...
//        }
    ...
    public void onImgItemDelete(int position) {
        ...
        if (isDeleteAble) {//此时为增加动画效果，刷新部分数据源，防止删除错乱
            ...
            ClearAssignMentSectionArrayList();
            componentTakeOffSectionAdapter.notifyItemRemoved(position-1);
            componentTakeOffSectionAdapter.notifyDataSetChanged();
...
//        comTaskBeans.add(bean1);
        ...
        ClearAssignMentSectionArrayList();
        componentTakeOffSectionAdapter.notifyItemInserted(comTaskBeans.size());
        componentTakeOffSectionAdapter.notifyDataSetChanged();
    }
...
//        startActivityForResult(intent, REQUST_QRCODE);
    ...
    public void BottomAction(){
        ...
        ApiWebService.Get_Production_order_yhy_Out_Mould_bypoid_Json(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                /**
...
//        }
        ...
        for (int i = 0; i < size; i++) {
            ComponentTakeOffBean confirmationReceBean = comTaskBeans.get(i);
            ...
            billtable[i][3] = App.menname;
            billtable[i][4] = confirmationReceBean.maintenanceOutsubDirectoryID;
            ...
            billtable[i][10] = confirmationReceBean.moduleSubDirectoryID;
            billtable[i][11] = confirmationReceBean.materialID;
            billtable[i][12] = confirmationReceBean.Qrcode;
            billtable[i][13] = confirmationReceBean.materialCoding;
            billtable[i][14] = confirmationReceBean.materialName;
            billtable[i][15] = confirmationReceBean.specification;
            billtable[i][16] = confirmationReceBean.measureUnitID;
            billtable[i][17] = confirmationReceBean.measureUnit;
            billtable[i][18] = confirmationReceBean.number + "";
        ...
        if (mStatus.isNewStatus()) {
            ...
            ApiWebService.Get_Production_order_yhy_Out_Mould_Add(this, new ApiWebService.SuccessCall() {
                @Override
                public void SuccessBack(String result) {
                    T.showShort(ComponentTakeOffContentMessageActivity.this, result);
...
//                sureDataRefresh("tvFanCheckAction");
    ...
    protected void tvDeleteAction(TextView tvDelete) {
        ApiWebService.Get_Production_order_yhy_Out_Mould_Del(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                T.showShort(ComponentTakeOffContentMessageActivity.this, result);
                onBackPressed();
//                sureDataRefresh("tvDeleteAction");
    ...
    protected final void action_operat_status() {
        Observable<Object> objectObservable = Observable.create(subscriber -> {
            ...
            setTvQrcodeContext("脱模",View.VISIBLE);
        });
