...
import com.system.bhouse.bhouse.CommonTask.TechnologyExecution.MaintenanceIntoWarehouse.MaintenanceWarehouseContentMessageActivity;
...
public class MaintainIntoWareHouseContentMessageActivity extends BaseContentMessageActivity implements  GroupItem.onChildItemClickListener, onMutiDataSetListener ,BaseQuickAdapter.OnItemChildClickListener,BaseContentMessageActivity.SetOnAddItemClickListener {

    public static final String TAG = "MaintainIntoWareHouse";
    ...
    private ArrayList<MaintenanceWarehouseBean> allComtaskBeans = new ArrayList<>();
    ...
    private MaintainIntoWareHouseSectionAdapter MaintainIntoWareHouseSectionAdapter;
    ...
    public void initComTaskActivity() {
        ...
        MaintainIntoWareHouseSectionAdapter = new MaintainIntoWareHouseSectionAdapter(R.layout.activity_comtask_content_layout_item, R.layout.layout_home_recommend_empty_noheight, R.layout.comtask_content_item_footer,assignmentBeanSectionArrayList,stringArray,mStatus);

        listView.setNestedScrollingEnabled(false);

        listView.setAdapter(MaintainIntoWareHouseSectionAdapter);
        listView.addItemDecoration(new TimeLineItemTopBottomDecoration());
        MaintainIntoWareHouseSectionAdapter.setOnItemChildClickListener(this);
...
//        outState.putParcelableArrayList(STATE_COMTASK, this.comTaskBeans);
    ...
    public void onChildItemClick(SortChildItem.ViewModel data, ViewHolder holder) {
        String type = "";
        if (data.name.equals("养护窑库"))
        {
            ApiWebService.Get_Production_order_yhy_In_wid_Json(this, new ApiWebService.SuccessCall() {
                @Override
                public void SuccessBack(String result) {
                   ArrayList<BProBOM> bProBOMS= App.getAppGson().fromJson(result, new TypeToken<List<BProBOM>>(){}.getType());

                    CommonPickerActivity_.intent(MaintainIntoWareHouseContentMessageActivity.this).title("养护窑库").bProBOMs(bProBOMS).LayoutID(R.layout.commonpicker_item).Position(holder.getAdapterPosition()).startForResult(RESULT_SORTITEM_SELECTPROJECT);
                }
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (viewEditText.getText() != null) {
                    ...
                    if (TextUtils.isEmpty(text)) {
                        T.showfunShort(MaintainIntoWareHouseContentMessageActivity.this, "备注不能为空");
                    }
                    else {
    ...
    void resultGetRequstQrcode(int result, Intent data) {
        if (result == RESULT_OK) {
            ...
            {
                ApiWebService.Get_Production_order_yhy_byTid_Json(this, new ApiWebService.SuccessCall() {
                    @Override
                    public void SuccessBack(String result) {
    ...
    void resultProjcetName(int result, Intent data) {
        if (result == RESULT_OK) {
            ...
            if (ValueUtils.IsFirstValueExist(comTaskBeans))
            {
                for (MaintenanceWarehouseBean bean : comTaskBeans)
                {
                    bean.wareHouse=extrasName;
                    bean.wareHouseID=extra;
                }
            }
            if (ValueUtils.IsFirstValueExist(allComtaskBeans))
            {
                for (MaintenanceWarehouseBean bean : allComtaskBeans)
                {
                    bean.wareHouse=extrasName;
                    bean.wareHouseID=extra;
                }
            ...
            getDateRefresh(extrasName,extraPosition,"养护窑库");
        }
    }
    ...
    public static ArrayList<MaintenanceWarehouseBean> removeDupliById(List<MaintenanceWarehouseBean> persons) {
        Set<MaintenanceWarehouseBean> personSet = new TreeSet<>((o1, o2) -> {
            if (TextUtils.isEmpty(o2.Qrcode))
            {
                return 0;
            }else {
                return 1;
    ...
    public static ArrayList<MaintenanceWarehouseBean> removeEmptyQrcode(ArrayList<MaintenanceWarehouseBean> persons)
    {
        for (Iterator<MaintenanceWarehouseBean> ite =persons.iterator();ite.hasNext();)
        {
            MaintenanceWarehouseBean next = ite.next();
    ...
    private void getDateRefresh(String date, int position, String typestring) {
        ...
        }else if (viewModel.name.equals("养护窑库"))
        {
            if (viewModel.value == null || !viewModel.value.equals(date))
                viewModel.value = date;
            for (MaintenanceWarehouseBean receBean : comTaskBeans) {
                receBean.wareHouse=viewModel.value;
            }
        }
...
//                getStaffData(position);
    ...
    private void getStaffData(int position) {
        ApiWebService.Get_Production_order_Per_Member_Json(this, new ApiWebService.SuccessCall() {
            ...
            public void SuccessBack(String result) {
                ...
                CommonPickerActivity_.intent(MaintainIntoWareHouseContentMessageActivity.this).title("职员信息").bProBOMs(bProBOMs).Position(position).LayoutID(R.layout.commonpicker_item).startForResult(RESULT_SORTITEM_SELECTPROJECT);
    ...
    private void testData() {
        comTaskBeans.clear();
        ApiWebService.Get_Production_order_yhyView_Json(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
            ...
            public void ErrorBack(String error) {
                ...
                //设置Empty  ErrorView
                MaintainIntoWareHouseSectionAdapter.setEmptyView(errorView);
            }
        },HId);
    }
    ...
    private void CleartreeRecyclerAdapter(){
        List<TreeItem> datas = treeRecyclerAdapter.getDatas();
        MaintenanceWarehouseContentMessageActivity.KeyType keyType = new MaintenanceWarehouseContentMessageActivity.KeyType();
...
//        }
    ...
    public void onImgItemDelete(int position) {
        ...
        if (isDeleteAble) {//此时为增加动画效果，刷新部分数据源，防止删除错乱
            ...
            ClearAssignMentSectionArrayList();
            MaintainIntoWareHouseSectionAdapter.notifyItemRemoved(position-1);
            MaintainIntoWareHouseSectionAdapter.notifyDataSetChanged();
...
//        comTaskBeans.add(bean1);
        ...
        ClearAssignMentSectionArrayList();
        MaintainIntoWareHouseSectionAdapter.notifyItemInserted(comTaskBeans.size());
        MaintainIntoWareHouseSectionAdapter.notifyDataSetChanged();
    }
...
//            TopListViewInit();
    ...
    protected void tvQrcodeAction(TextView tvQrcode) {
        ...
        },comTaskBeans.get(0).stationCarID,comTaskBeans.get(0).stationCarCoding,comTaskBeans.get(0).stationCarName,comTaskBeans.get(0).wareHouseID);
...
//        startActivityForResult(intent, REQUST_QRCODE);
    ...
    /**
     * 访问下面recycleView数据 根据生产订单和二维码获取托盘配料单信息  根据台车 二维码
     */
...
//        }
        ...
        for (int i = 0; i < size; i++) {
            ...
            billtable[i][1] = confirmationReceBean.stationCarID;
            billtable[i][2] = confirmationReceBean.wareHouseID;
            billtable[i][3] = App.menname;
            billtable[i][4] = confirmationReceBean.orderId;
            billtable[i][5] = confirmationReceBean.moduleID;
            billtable[i][6] = confirmationReceBean.maintenanceStartTime;
            billtable[i][7] = confirmationReceBean.maintenanceEndTime;
            billtable[i][8] = confirmationReceBean.stationAssigmentSublistID;
            billtable[i][9] = confirmationReceBean.materialID;
            billtable[i][10] = confirmationReceBean.Qrcode;
            billtable[i][11] = confirmationReceBean.materialCoding;
            billtable[i][12] = confirmationReceBean.materialName;
            billtable[i][13] = confirmationReceBean.specification;
            billtable[i][14] = confirmationReceBean.measureUnitID;
            billtable[i][15] = confirmationReceBean.measureUnit;
            billtable[i][16] = confirmationReceBean.number+"";
            billtable[i][17] = confirmationReceBean.sourceID;
            billtable[i][18] = confirmationReceBean.preAllocationWarehouseID;
            billtable[i][19] = confirmationReceBean.stationCarCoding;
            billtable[i][20] = confirmationReceBean.moduleCoding;
        }
        if (mStatus.isNewStatus()) {
...
//                sureDataRefresh("tvFanCheckAction");
    ...
    protected void tvDeleteAction(TextView tvDelete) {
        ApiWebService.Get_Production_order_yhy_In_Del(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
              showButtomToast(result);
                onBackPressed();
