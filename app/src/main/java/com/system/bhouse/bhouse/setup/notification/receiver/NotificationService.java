...
public class NotificationService {
    ...
    public void save(XGNotification notification) {
        ...
        values.put("isnew",notification.isNews());
    ...
    //这里的isnew  true就是1  false就是0
    public int findColumCount(String isnew) {
        SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
        Cursor cursor = db.query("notification", new String[]{"count (isnew)"}, "isnew=?", new String[]{isnew}, null, null, null);
        try {
            if (cursor.moveToFirst()) {
                return cursor.getInt(cursor.getColumnIndex("count (isnew)"));
            }
            return -1;
        } finally {
            cursor.close();
