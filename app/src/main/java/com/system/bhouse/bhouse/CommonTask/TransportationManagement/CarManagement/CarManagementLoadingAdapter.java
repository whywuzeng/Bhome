...
public class CarManagementLoadingAdapter extends LoadingAdapter<CarManageBean> {

    private ArrayList<CarManageBean> mComTaskBeans = new ArrayList<>();

    public CarManagementLoadingAdapter(ArrayList<CarManageBean> mComTaskBeans) {
    ...
    protected void onBindHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof ItemViewHolder) {
            ...
            itemViewHolder.llBottom4.setVisibility(View.GONE);
            itemViewHolder.llBottom5.setVisibility(View.GONE);
            itemViewHolder.llBottom6.setVisibility(View.GONE);
...
// .get(position).getID() + "").IsNew(false).start();
    ...
    private List<String> getListStrings(int position) {
        ...
        dataList.add(mComTaskBeans
                .get(position).getCarNo() == null ? "" : mComTaskBeans
                .get(position).getCarNo());
        dataList.add("货柜数:");
        dataList.add(mComTaskBeans
                .get(position).getContainernum() + "");
//        dataList.add("备注:");
//        dataList.add(mComTaskBeans
