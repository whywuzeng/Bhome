package com.system.bhouse.bhouse.CommonTask.TechnologyExecution.StationCarAssignMent;
...
import com.system.bhouse.bhouse.CommonTask.TechnologyExecution.StationCarAssignMent.Bean.StationCarAssignMentBean;
import com.system.bhouse.bhouse.CommonTask.TechnologyExecution.StationCarAssignMent.Bean.StationCarAssignMentBeanSection;
...
public class StationCarAssignMentContentMessageActivity extends BaseContentMessageActivity implements  GroupItem.onChildItemClickListener, onMutiDataSetListener ,BaseQuickAdapter.OnItemChildClickListener,BaseContentMessageActivity.SetOnAddItemClickListener {

    public static final String TAG = "comtaskcontentmessageactivity";
    private static final String module_name = "台车分配";
    ...
    private ArrayList<StationCarAssignMentBean> comTaskBeans = new ArrayList<>();
    private ArrayList<StationCarAssignMentBeanSection> assignmentBeanSectionArrayList=new ArrayList<>() ;
    ...
    private StationCarAssignMentSectionAdapter stationCarAssignMentSectionAdapter;
    ...
    public void initComTaskActivity() {
        ...
        String[] stringArray = getResources().getStringArray(R.array.stationcar_itemsection_order);

        stationCarAssignMentSectionAdapter = new StationCarAssignMentSectionAdapter(R.layout.activity_comtask_content_layout_item, R.layout.layout_home_recommend_empty_noheight, R.layout.comtask_content_item_footer,assignmentBeanSectionArrayList,stringArray,mStatus);

        listView.setNestedScrollingEnabled(false);

        listView.setAdapter(stationCarAssignMentSectionAdapter);
        listView.addItemDecoration(new TimeLineItemTopBottomDecoration());
        stationCarAssignMentSectionAdapter.setOnItemChildClickListener(this);
...
//            this.comTaskBeans = savedInstanceState.getParcelableArrayList(STATE_COMTASK);
    ...
    private void initStatusData(){
        ...
        if (!TextUtils.isEmpty(receiptHnumber)) {
            if (getComtaskSize()) {
                for (StationCarAssignMentBean bean : comTaskBeans) {
    ...
    private void TopListViewInit() {
        ...
        ArrayList<StationCarAssignMentContentMessageActivity.KeyType> keyTypes = new ArrayList<>();
        StationCarAssignMentContentMessageActivity.KeyType keyType = null;
        for (int i = 0; i < Const.LETTERS.length; i++) {
            keyType = new StationCarAssignMentContentMessageActivity.KeyType();
     ...
     * @return
     */
    protected ArrayList<SortChildItem.ViewModel> makeChlidData(StationCarAssignMentContentMessageActivity.KeyType data, StatusBean mStatus) {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (viewEditText.getText() != null) {
                    ...
                    if (TextUtils.isEmpty(text)) {
                        T.showfunShort(StationCarAssignMentContentMessageActivity.this, "备注不能为空");
                    }
                    else {
            ...
            if (extraPosition==-1) {
                ApiWebService.Get_Production_order_Mould_bypoid_Json(this, new ApiWebService.SuccessCall() {
                    ...
                    public void SuccessBack(String result) {
                        ...
                        ArrayList<StationCarAssignMentBean> loadingcarbean = App.getAppGson().fromJson(result, new TypeToken<List<StationCarAssignMentBean>>() {
                        }.getType());
                        ...
                        //清空 二维码为空的
                        for (StationCarAssignMentBean receBean : comTaskBeans) {
                        ...
                        ArrayList<StationCarAssignMentBean> clone =(ArrayList<StationCarAssignMentBean>)comTaskBeans.clone();
                        comTaskBeans.clear();
                        comTaskBeans.addAll(removeDupliById(clone));
                        ClearAssignMentSectionArrayList();
                        stationCarAssignMentSectionAdapter.notifyDataSetChanged();
                    }
...
//
//            ClearAssignMentSectionArrayList();
//            stationCarAssignMentSectionAdapter.notifyItemChanged(extraPosition);
        }
    }

    public static ArrayList<StationCarAssignMentBean> removeDupliById(List<StationCarAssignMentBean> persons) {
        Set<StationCarAssignMentBean> personSet = new TreeSet<>((o1, o2) -> {
...
//                getStaffData(position);
    ...
    private void getStaffData(int position) {
        ApiWebService.Get_Production_order_Per_Member_Json(this, new ApiWebService.SuccessCall() {
            ...
            public void SuccessBack(String result) {
                ...
                CommonPickerActivity_.intent(StationCarAssignMentContentMessageActivity.this).title("职员信息").bProBOMs(bProBOMs).Position(position).LayoutID(R.layout.commonpicker_item).startForResult(RESULT_SORTITEM_SELECTPROJECT);
    ...
    private void TvAddQrAction() {
        ApiWebService.Get_Production_order_Trolley_widfp_Json(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
    ...
    private void testData() {
        comTaskBeans.clear();
        ApiWebService.Get_Production_order_TrolleyView_Json_poidprid_QR_Code(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                ArrayList<StationCarAssignMentBean> tomTaskBeans = App.getAppGson().fromJson(result, new TypeToken<List<StationCarAssignMentBean>>() {
                }.getType());
                //为空就创建一个新的空对象
                if (tomTaskBeans.isEmpty()) {
                    //为头recycleView 设置空数据
                    comTaskBeans.add(new StationCarAssignMentBean());
                }
                else {
            ...
            public void ErrorBack(String error) {
                ...
                //设置Empty  ErrorView
                stationCarAssignMentSectionAdapter.setEmptyView(errorView);
            }
        },orderId,componentQr);
    }
...
//        treeRecyclerAdapter.getItemManager().addItems(datas);
    ...
    private void ClearAssignMentSectionArrayList() {
        ...
        for(StationCarAssignMentBean bean : comTaskBeans)
        {
            assignmentBeanSectionArrayList.add(new StationCarAssignMentBeanSection(bean));
        }
        assignmentBeanSectionArrayList.add(new StationCarAssignMentBeanSection(false,"",false,true,""));
    }
...
//        }
    ...
    public void onImgItemDelete(int position) {
        ...
        if (isDeleteAble) {//此时为增加动画效果，刷新部分数据源，防止删除错乱
            ...
            ClearAssignMentSectionArrayList();
            stationCarAssignMentSectionAdapter.notifyItemRemoved(position-1);
            stationCarAssignMentSectionAdapter.notifyDataSetChanged();
...
//        comTaskBeans.add(bean1);
        ...
        ClearAssignMentSectionArrayList();
        stationCarAssignMentSectionAdapter.notifyItemInserted(comTaskBeans.size());
        stationCarAssignMentSectionAdapter.notifyDataSetChanged();
    }
...
//            TopListViewInit();
    ...
    protected void tvQrcodeAction(TextView tvQrcode) {
        ...
        ApiWebService.Get_Production_order_Trolley_Trolleyfp_Json(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
...
//                [{"台车ID":"e5ddba4fada141539f87516c51b3e012","台车名称":"9号台车"}]
                ...
                if (!ValueUtils.IsFirstValueExist(loadingcarbean))
                    return;
                comTaskBeans.get(0).setStationCarID(loadingcarbean.get(0).stationCarID);
                comTaskBeans.get(0).setStationCarName(loadingcarbean.get(0).stationCarName);
        ...
        }, orderId, componentQr, comTaskBeans.get(0).ID, comTaskBeans.get(0).startTime, comTaskBeans.get(0).endTime);
...
//        startActivityForResult(intent, REQUST_QRCODE);
    ...
    /**
     * 访问下面recycleView数据 根据生产订单和二维码获取需要分配台车的构件模具信息
     */
    public void BottomAction(){

        ApiWebService.Get_Production_order_Trolley_bypoid_Json(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
...
//        }
        ...
        for (int i = 0; i < size; i++) {
            StationCarAssignMentBean confirmationReceBean = comTaskBeans.get(i);
        ...
        if (mStatus.isNewStatus()) {
            ...
            ApiWebService.Get_Production_order_Trolley_Add(this, new ApiWebService.SuccessCall() {
                @Override
                public void SuccessBack(String result) {
                    T.showShort(StationCarAssignMentContentMessageActivity.this, result);
...
//                sureDataRefresh("tvFanCheckAction");
    ...
    protected void tvDeleteAction(TextView tvDelete) {
        ApiWebService.Get_Production_order_Trolley_Del(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                T.showShort(StationCarAssignMentContentMessageActivity.this, result);
                onBackPressed();
