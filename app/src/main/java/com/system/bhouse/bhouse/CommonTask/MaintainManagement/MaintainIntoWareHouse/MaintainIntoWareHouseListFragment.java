...
public class MaintainIntoWareHouseListFragment extends BaseCommonListFragment<MaintainIntoWareHouseLoadingAdapter,MaintenanceWarehouseBean> implements MaintainIntoWareHouseLoadingAdapter.onItemClickListener {

    MaintainIntoWareHouseLoadingAdapter mAdapter;
    ...
    protected Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == REFRESH_DATA) {
                setRefreshing(false);
                ArrayList<MaintenanceWarehouseBean> loadedRequires = msg.getData().getParcelableArrayList(LOADEDREQUIREKEY);
    ...
    {
        initRefreshLayout();
        mAdapter = new MaintainIntoWareHouseLoadingAdapter(mData);
    ...
    protected void loadData() {
        if (mUpdateAll) {
            mUpdateAll = false;
            ApiWebService.Get_Production_order_yhy_In_Json(getActivity(), new ApiWebService.SuccessCall() {

                @Override
                public void SuccessBack(String result) {
                    L.e(result);
                    ArrayList<MaintenanceWarehouseBean> loadedRequires = App.getAppGson().fromJson(result, new TypeToken<List<MaintenanceWarehouseBean>>() {
                    }.getType());
...
//            },"d76c39864d3c4397859e70d008719489");
    ...
    //list Item的点击事件
    @Override
    public void ItemClick(MaintainIntoWareHouseLoadingAdapter.ItemViewHolder holder, int position) {
        mNeedUpdate = true;
        ...
        MaintainIntoWareHouseContentMessageActivity_.intent(getParentFragment()).HId(mData
