...
public class LoadingIntoWareHouseLoadingAdapter extends LoadingAdapter<LoadingIntoWareHouseBean> {

    private final String[] TitlestringArray;
    private ArrayList<LoadingIntoWareHouseBean> mComTaskBeans = new ArrayList<>();

    public LoadingIntoWareHouseLoadingAdapter(ArrayList<LoadingIntoWareHouseBean> mComTaskBeans) {
        this.mComTaskBeans = mComTaskBeans;
        TitlestringArray = App.getContextApp().getResources().getStringArray(R.array.loadingwarehouse_detach);
    }
...
//        titlesTag.get(5).setText("业务日期");
    ...
    private List<String> getListStrings(int position, List<TextView> tagContent) {
        ...
        tagContent.get(1).setText(mComTaskBeans
                .get(position).getStorageWarehouse() == null ? "" : mComTaskBeans
                .get(position).getStorageWarehouse());
        ...
        tagContent.get(3).setText(mComTaskBeans
                .get(position).getContainerName() == null ? "" : mComTaskBeans
                .get(position).getContainerName());

        tagContent.get(4).setText(mComTaskBeans
                .get(position).status == null ? "" : mComTaskBeans
                .get(position).status);
...
//                .get(position).getEntryTime());
    ...
    public class ItemViewHolder extends RecyclerView.ViewHolder {
        ...
        @Bind(R.id.ll_text_tableId_1)
        LinearLayout ll_text_tableId_1;
        @Bind(R.id.ll_text_projectName_2)
        LinearLayout ll_text_projectName_2;
        @Bind(R.id.ll_text_dong_3)
