package com.system.bhouse.bhouse.CommonTask.TechnologyExecution.ModuleFeeding;
...
public class ModuleFeedingContentMessageActivity extends BaseContentMessageActivity implements  GroupItem.onChildItemClickListener, onMutiDataSetListener ,BaseQuickAdapter.OnItemChildClickListener,BaseContentMessageActivity.SetOnAddItemClickListener {

    public static final String TAG = "comtaskcontentmessageactivity";
    private static final String module_name = "模具投料";
    ...
    private ArrayList<ModuleFeedingBean> comTaskBeans = new ArrayList<>();
    private ArrayList<ModuleFeedingBeanSection> assignmentBeanSectionArrayList=new ArrayList<>() ;
    ...
    private ModuleFeedingSectionAdapter moduleFeedingSectionAdapter;
    ...
    public void initComTaskActivity() {
        ...
        String[] stringArray = getResources().getStringArray(R.array.modulefeeding_itemsection_order);

        moduleFeedingSectionAdapter = new ModuleFeedingSectionAdapter(R.layout.activity_comtask_content_layout_item, R.layout.layout_home_recommend_empty_noheight, R.layout.comtask_content_item_footer,assignmentBeanSectionArrayList,stringArray,mStatus);

        listView.setNestedScrollingEnabled(false);

        listView.setAdapter(moduleFeedingSectionAdapter);
        listView.addItemDecoration(new TimeLineItemTopBottomDecoration());
        moduleFeedingSectionAdapter.setOnItemChildClickListener(this);
...
//            this.comTaskBeans = savedInstanceState.getParcelableArrayList(STATE_COMTASK);
    ...
    private void initStatusData(){
        ...
        if (!TextUtils.isEmpty(receiptHnumber)) {
            if (getComtaskSize()) {
                for (ModuleFeedingBean bean : comTaskBeans) {
    ...
    private void TopListViewInit() {
        ...
        ArrayList<ModuleFeedingContentMessageActivity.KeyType> keyTypes = new ArrayList<>();
        ModuleFeedingContentMessageActivity.KeyType keyType = null;
        for (int i = 0; i < Const.LETTERS.length; i++) {
            keyType = new ModuleFeedingContentMessageActivity.KeyType();
     ...
     * @return
     */
    protected ArrayList<SortChildItem.ViewModel> makeChlidData(ModuleFeedingContentMessageActivity.KeyType data, StatusBean mStatus) {
        ...
        if (data.key == 4) {
            ...
            viewModel.name = "模具";
            viewModel.value = this.comTaskBeans.get(0).getModuleName();
            viewModel.key = "moduleName";
            viewModel.isClick = false;
            viewModels.add(viewModel);
            headerProperties.put(viewModel.key,viewModel.value);

            viewModel = new SortChildItem.ViewModel();
            viewModel.name = "托盘配料单";
            ...
            viewModel = new SortChildItem.ViewModel();
            viewModel.name = "投料日期";
            viewModel.value = this.comTaskBeans.get(0).getFeedingTime();
            viewModel.key = "feedingTime";
            viewModel.isClick = false;
            viewModels.add(viewModel);
            headerProperties.put(viewModel.key,viewModel.value);
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                if (viewEditText.getText() != null) {
                    ...
                    if (TextUtils.isEmpty(text)) {
                        T.showfunShort(ModuleFeedingContentMessageActivity.this, "备注不能为空");
                    }
                    else {
            ...
            if (extraPosition==-1) {
                ApiWebService.Get_Production_order_Mould_bypoid_Json(this, new ApiWebService.SuccessCall() {
                    ...
                    public void SuccessBack(String result) {
                        ...
                        ArrayList<ModuleFeedingBean> loadingcarbean = App.getAppGson().fromJson(result, new TypeToken<List<ModuleFeedingBean>>() {
                        }.getType());
                        ...
                        //清空 二维码为空的
                        for (ModuleFeedingBean receBean : comTaskBeans) {
                        ...
                        ArrayList<ModuleFeedingBean> clone =(ArrayList<ModuleFeedingBean>)comTaskBeans.clone();
                        comTaskBeans.clear();
                        comTaskBeans.addAll(removeDupliById(clone));
                        ClearAssignMentSectionArrayList();
                        moduleFeedingSectionAdapter.notifyDataSetChanged();
                    }
...
//
//            ClearAssignMentSectionArrayList();
//            moduleFeedingSectionAdapter.notifyItemChanged(extraPosition);
        }
    }

    public static ArrayList<ModuleFeedingBean> removeDupliById(List<ModuleFeedingBean> persons) {
        Set<ModuleFeedingBean> personSet = new TreeSet<>((o1, o2) -> {
    ...
    private void getStaffData(int position) {
        ApiWebService.Get_Production_order_Per_Member_Json(this, new ApiWebService.SuccessCall() {
            ...
            public void SuccessBack(String result) {
                ...
                CommonPickerActivity_.intent(ModuleFeedingContentMessageActivity.this).title("职员信息").bProBOMs(bProBOMs).Position(position).LayoutID(R.layout.commonpicker_item).startForResult(RESULT_SORTITEM_SELECTPROJECT);
...
//                moduleFeedingSectionAdapter.notifyDataSetChanged();
        ...
        }, comTaskBeans.get(0).ID,comTaskBeans.get(0).productionLineID);
    }
    ...
    private void testData() {
        comTaskBeans.clear();
        ApiWebService.Get_Production_order_Tray_Is_Moid_qrView_Json_poidprid_QR_Code(this, new ApiWebService.SuccessCall() {
            @Override
            public void SuccessBack(String result) {
                ArrayList<ModuleFeedingBean> tomTaskBeans = App.getAppGson().fromJson(result, new TypeToken<List<ModuleFeedingBean>>() {}.getType());
                //为空就创建一个新的空对象
                if (tomTaskBeans.isEmpty()) {
                    //为头recycleView 设置空数据
                    comTaskBeans.add(new ModuleFeedingBean());
                }
                else {
            ...
            public void ErrorBack(String error) {
                ...
                //设置Empty  ErrorView
                moduleFeedingSectionAdapter.setEmptyView(errorView);
            }
        },orderId,componentQr);
    }
...
//      treeRecyclerAdapter.getItemManager().addItems(datas);
    ...
    private void ClearAssignMentSectionArrayList() {
        ...
        for (ModuleFeedingBean bean : comTaskBeans) {
            assignmentBeanSectionArrayList.add(new ModuleFeedingBeanSection(bean));
        }
        assignmentBeanSectionArrayList.add(new ModuleFeedingBeanSection(false, "", false, true, ""));
    }
...
//        }
    ...
    public void onImgItemDelete(int position) {
        ...
        if (isDeleteAble) {//此时为增加动画效果，刷新部分数据源，防止删除错乱
            ...
            ClearAssignMentSectionArrayList();
            moduleFeedingSectionAdapter.notifyItemRemoved(position-1);
            moduleFeedingSectionAdapter.notifyDataSetChanged();
...
//        comTaskBeans.add(bean1);
        ...
        ClearAssignMentSectionArrayList();
        moduleFeedingSectionAdapter.notifyItemInserted(comTaskBeans.size());
        moduleFeedingSectionAdapter.notifyDataSetChanged();
    }
...
//        }
        ...
        for (int i = 0; i < size; i++) {
            ModuleFeedingBean confirmationReceBean = comTaskBeans.get(i);
        ...
        if (mStatus.isNewStatus()) {
            ...
            ApiWebService.Get_Production_order_Trolley_Add(this, new ApiWebService.SuccessCall() {
                @Override
                public void SuccessBack(String result) {
                    T.showShort(ModuleFeedingContentMessageActivity.this, result);
