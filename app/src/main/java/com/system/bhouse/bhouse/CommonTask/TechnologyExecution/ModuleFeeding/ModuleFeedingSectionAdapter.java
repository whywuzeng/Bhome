...
public class ModuleFeedingSectionAdapter extends HeaderAndFooterSectionQuickAdapter<ModuleFeedingBeanSection, BaseViewHolder>{
    ...
    public ModuleFeedingSectionAdapter(int layoutResId, int sectionHeadResId, int sectionFooterResId, List data, String[] stringArray, StatusBean statusBean) {
    ...
    @Override
    protected void convertHead(BaseViewHolder helper, ModuleFeedingBeanSection item) {

    }

    @Override
    protected void converFooter(BaseViewHolder helper, ModuleFeedingBeanSection item) {
        helper.setVisible(R.id.btn_addItem, false);
        helper.addOnClickListener(R.id.btn_addItem);
    }

    @Override
    protected void convert(BaseViewHolder helper, ModuleFeedingBeanSection item) {
     ...
     */
    private void initDataForView(BaseAssignViewHolder moduleAssignViewHolder, ModuleFeedingBeanSection item) {
        ...
        contvs.get(5).setText(item.t.moduleName);
        contvs.get(6).setText(item.t.stationCarName);
        contvs.get(7).setText(item.t.number+"");
