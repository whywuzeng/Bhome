...
public class MyPagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public MyPagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        myFirstExtraPdddingLeft = getResources().getDimensionPixelSize(R.dimen.my_pager_sliding_tab_strip_first_padding_left);

        myPaddingLeft = getResources().getDimensionPixelSize(R.dimen.my_pager_sliding_tab_strip);
        myPaddingLeft = 0;
...
//        tabTextColor = a.getColor(R.styleable., tabTextColor);
//
//        a.recycle();
...
//
//		addTab(position, tab);
        View v = mInflater.inflate(R.layout.imageview_head, tabsContainer, false);
        ImageView head = (ImageView) v.findViewById(R.id.head);
        if (url.isEmpty()) {
            head.setImageResource(R.drawable.allpic);
//            head.setImageResource(R.drawable.icon_all_task);
...
//            ImageLoader.getInstance().displayImage(Global.makeSmallUrl(head, url), head, ImageLoadTool.options);
            head.setImageDrawable(getResources().getDrawable(R.drawable.allpic));
        }
