...
public class StationCarDetachModuleLoadingAdapter extends LoadingAdapter<StationCarDetachModuleBean> {

    private final String[] TitlestringArray;
    private ArrayList<StationCarDetachModuleBean> mComTaskBeans = new ArrayList<>();

    public StationCarDetachModuleLoadingAdapter(ArrayList<StationCarDetachModuleBean> mComTaskBeans) {
        this.mComTaskBeans = mComTaskBeans;
        TitlestringArray = App.getContextApp().getResources().getStringArray(R.array.stationmodule_detach);
    }
...
//        titlesTag.get(5).setText("业务日期");
    ...
    private List<String> getListStrings(int position, List<TextView> tagContent) {
        ...
        tagContent.get(3).setText(mComTaskBeans
                .get(position).moduleName == null ? "" : mComTaskBeans
                .get(position).moduleName);
