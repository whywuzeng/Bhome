...
public class ComponentTakeOffSectionAdapter extends HeaderAndFooterSectionQuickAdapter<ComponentTakeOffBeanSection, BaseViewHolder>{
    ...
    public ComponentTakeOffSectionAdapter(int layoutResId, int sectionHeadResId, int sectionFooterResId, List data, String[] stringArray, StatusBean statusBean) {
    ...
    @Override
    protected void convertHead(BaseViewHolder helper, ComponentTakeOffBeanSection item) {

    }

    @Override
    protected void converFooter(BaseViewHolder helper, ComponentTakeOffBeanSection item) {
        helper.setVisible(R.id.btn_addItem, false);
        helper.addOnClickListener(R.id.btn_addItem);
    }

    @Override
    protected void convert(BaseViewHolder helper, ComponentTakeOffBeanSection item) {
     ...
     */
    private void initDataForView(BaseAssignViewHolder moduleAssignViewHolder, ComponentTakeOffBeanSection item) {
        ...
        contvs.get(0).setText(item.t.moduleName);
        contvs.get(1).setText(item.t.maintenanceStartTime);
        contvs.get(2).setText(item.t.maintenanceEndTime);
        contvs.get(3).setText(item.t.Qrcode);
        contvs.get(4).setText(item.t.materialCoding);
        contvs.get(5).setText(item.t.materialName);
        contvs.get(6).setText(item.t.specification);
        contvs.get(7).setText(item.t.measureUnit);
